                            ...
                            <div class="table-responsive shopping-cart">
                                <table class="table">
                                    ...
                                    @foreach (session('shopping_wishlist') as $row)
                                        @php
                                            $pro_price = $row['price'];
                                            $price = $gtext['currency_position'] == 'left'
                                                ? $gtext['currency_icon'].NumberFormat($pro_price)
                                                : NumberFormat($pro_price).$gtext['currency_icon'];
                                        @endphp
                                        <tr id="row_delete_{{ $row['id'] }}">
                                            ...
                                            <!-- Qty -->
                                            <td class="text-center" data-title="{{ __('Qty') }}:">
                                                <input type="number"
                                                       id="wl_qty_{{ $row['id'] }}"
...
{{--                                                       onclick="onAddWishlistToCart({{ $row['id'] }})">--}}
{{--                                                        {{ __('Add to Cart') }}--}}
{{--                                                    </a>--}}
                                                    <a href="javascript:void(0)"
                                                       class="btn btn-sm theme-btn"
                                                       onclick="onMoveWishlistToCart({{ $row['id'] }})">
                                                        {{ __('Move to Cart') }}
                                                    </a>
{{--                                                    <a class="btn btn-sm theme-btn" href="{{ route('frontend.product', [$row['id'], str_slug($row['name'])]) }}">--}}
{{--                                                        {{ __('View') }}--}}
{{--                                                    </a>--}}
                                                </div>
                                            </td>
