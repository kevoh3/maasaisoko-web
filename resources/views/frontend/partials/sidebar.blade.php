    ...
    <script>
        (function(){
            ...
            document.addEventListener('click', (e)=>{
                if (!cascader.contains(e.target)) close();
            });
            ...
            async function fetchChildren(parentId){
                const url = @json(route('geo.children'));
            ...
            cascader.querySelectorAll('.county').forEach(li=>{
                li.addEventListener('mouseenter', async ()=>{
                    ...
                    const children = await fetchChildren(pid);
                    if(children.length){
                        fillList(colConst, children, 'constituency');
                    }else{
            ...
            listConst.addEventListener('mouseenter', (e)=>{
                ...
                (async ()=>{
                    ...
                    const children = await fetchChildren(pid);
                    if(children.length){
                        fillList(colWards, children, 'ward');
                    }else{
            ...
            // If a geo is already selected, auto-open and pre-load its siblings path (optional)
            @if($selectedGeo)
            open();
            (async ()=>{
                // load the selected county’s constits so user can refine quickly
                // find county ancestor (walk up until level=county would be ideal;
                // here we just try to load children for the current selected's parent, if any)
                // minimal preload: load children of the county that contains the selected geo
                // (safe to no-op if not found)
                // You can enhance by returning ancestor ids from backend, but this is optional UX sugar.
            })();
            @endif
        })();
    </script>
