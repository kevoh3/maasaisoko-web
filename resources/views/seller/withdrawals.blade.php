...
{{--				</div>--}}
                <div class="status-card bg-grad-5 mb-15">
                    <div class="status-text">
                        {{-- Big balance line (keeps same id for existing JS) --}}
                        <h2 class="status-count" id="Current_Balance_new">
                            {{ ($biData['currency'] ?? 'KES') }}
                            {{ number_format((float)($biData['balance'] ?? 0), 2) }}
                        </h2>

                        {{-- Hidden field preserved (if other scripts rely on it) --}}
{{--                        <input type="hidden" name="ubalance" id="ubalance" value="{{ (float)($biData['balance'] ?? 0) }}" />--}}

                        {{-- Details --}}
                        <div class="mt-10">
                            <p class="status-text">
                                <strong>{{ __('Account Name') }}</strong>:
                                <span class="status-count"> {{ $biData['wallet_name'] ?? '—' }}</span>
                            </p>
                            <p class="status-text">
                                {{ __('Account Number') }}:
                                <span class="status-count">{{ $biData['account_number'] ?? '—' }}</span>
                            </p>
                        </div>
                    </div>

                    {{-- keep your decorative svg --}}
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 200">
