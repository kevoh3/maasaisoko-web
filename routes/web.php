...
Route::prefix('backend')->group(function () {
    ...
    //Sellers Page
    Route::get('/groups', [App\Http\Controllers\Backend\GroupsController::class, 'index'])->name('backend.groups')->middleware(['auth', 'is_admin']);
    ...
    Route::get('/getGroupsTableData', [App\Http\Controllers\Backend\GroupsController::class, 'getGroupsTableData'])->name('backend.getGroupsTableData')->middleware(['auth', 'is_admin']);
    Route::post('/saveGroupData', [App\Http\Controllers\Backend\GroupsController::class, 'saveGroupData'])->name('backend.saveGroupData')->middleware(['auth', 'is_admin']);
    Route::post('/getGroupById', [App\Http\Controllers\Backend\GroupsController::class, 'getGroupById'])->name('backend.getGroupById')->middleware(['auth', 'is_admin']);
//    Route::resource('packages', App\Http\Controllers\Backend\PackageController::class)->middleware(['auth', 'is_admin']);
    Route::resource(
        'packages',
        App\Http\Controllers\Backend\PackageController::class
    )->middleware(['auth', 'is_admin'])
        ->names([
            'index'   => 'backend.packages.index',
            'create'  => 'backend.packages.create',
            'store'   => 'backend.packages.store',
            'show'    => 'backend.packages.show',
            'edit'    => 'backend.packages.edit',
            'update'  => 'backend.packages.update',
            'destroy' => 'backend.packages.destroy',
        ]);
//    POST / backend / saveGroupData → create / update
//
//POST / backend / getGroupById → load into form for edit
//
//                                                   POST / backend / deleteGroup
//
//                                                   POST / backend / bulkActionGroups
