...
class CheckoutFrontController extends Controller
{
    ...
    public function LoadMakeOrder(Request $request)
    {
        ...
        // Lightweight server-side trace of checkout attempt
        try {
            \Log::info('Checkout submit clicked', [
                'user_id' => auth()->id(),
                'ip' => $request->ip(),
            ...
            // swallow logging errors; checkout must proceed
        }
        ...
        // Determine if shipping selection is required (only when there are published shipping methods)
        $shippingSelectionRequired = Shipping::where('is_publish', 1)->exists();
        ...
        try {
            \Log::info('Checkout payment method selected', [
                'user_id' => auth()->id(),
		...
		if($index>0){
			...
			//Stripe - COMMENTED OUT (SasaPay only)
			/*
			...
			*/
			...
			//Paypal - COMMENTED OUT (SasaPay only)
			/*
			...
			*/
			...
			//Razorpay - COMMENTED OUT (SasaPay only)
			/*
			...
			*/
			...
			//Mollie - COMMENTED OUT (SasaPay only)
			/*
			...
			*/
            ...
            //SasaPay - ONLY PAYMENT METHOD
            if($payment_method_id == 8){
                \Log::info('SasaPay condition matched - entering SasaPay block', [
                    'payment_method_id' => $payment_method_id,
                    'payment_method_id_type' => gettype($payment_method_id),
                    'payment_method_id_value' => var_export($payment_method_id, true),
                    'condition_check' => ($payment_method_id == 8),
                    'strict_condition_check' => ($payment_method_id === 8)
                ]);
                $intent = '';

                \Log::info('SasaPay debug', [
                    'payment_method_id' => $payment_method_id,
                    'gtext_isenable_sasapay' => $gtext['isenable_sasapay'] ?? 'not_set',
                    'isset_check' => isset($gtext['isenable_sasapay']),
                    'value_check' => ($gtext['isenable_sasapay'] ?? null) == 1
                ]);

                // Always prepare SasaPay redirect to hosted checkout form
                $firstOrderId = collect($MasterData)->values()->first();
                $redirectUrl = route('frontend.sasapay.payment', ['order_id' => $firstOrderId]);
                try {
                    \Log::info('SasaPay initiate redirect prepared', [
                        'user_id' => auth()->id(),
                ...
                $res['msgType'] = 'success';
                $res['msg'] = __('Your order is created. Please complete payment.');
                $res['redirect_url'] = $redirectUrl;
                
                \Log::info('SasaPay: About to return redirect response', [
                    'user_id' => auth()->id(),
                    'order_id' => $firstOrderId,
                    'redirect_url' => $redirectUrl,
                    'response_data' => $res
			...
			if($payment_method_id != 4 && $payment_method_id != 8){
			...
			// For SasaPay (method 8), don't clear cart yet - wait for successful payment
			// Cart will be cleared in SasaPayController::paymentSuccess()

            // If we reach here with SasaPay selected, treat as an error (no redirect was prepared)
            if ($payment_method_id == 8) {
                \Log::error('SasaPay initiation fell through without redirect', [
                    'user_id' => auth()->id(),
                    'order_ids' => $MasterData,
                ]);
                return response()->json([
                    'msgType' => 'error',
                    'msg' => ['oneError' => [__('SasaPay could not be initiated. No redirect was prepared.')]]
                ], 400);
            }
