...
class ProductsController extends Controller
{
    ...
    public function categoryChildren(Request $request)
    {
        ...
        $children = Pro_category::where('is_publish', 1)
    ...
    public function getCategoryChildren(Request $request)
    {
        ...
        // Optional: honor language if you pass ?lan=xx
...
//		$user_id = $request->input('user_id');
        $id       = $request->input('RecordId'); // empty on create
        $title    = esc($request->input('title'));
        $slug     = esc(str_slug($request->input('slug')));
        $lan      = $request->input('lan') ?: glan(); // default language
        $cat_id   = (int) $request->input('categoryid');  // final (child or parent)
        $brand_id = $request->input('brandid') ?: null;
        $user_id  = Auth::id(); // safer than trusting hidden input
...
//		);
        $validator = Validator::make([
            ...
            'categoryid'   => 'required|integer|exists:pro_categories,id',
            // brand can be null or 0 (“No Brand”), no strict rule needed
        ]);
...
//			'cat_id' => $cat_id,
//			'category_ids' => $cat_id,
            'cat_id'       => $cat_id,       // final category (child if exists, else parent)
...
//    }
//gett Product
    public function getProductPageDataManage($id){
        //dd($id);
