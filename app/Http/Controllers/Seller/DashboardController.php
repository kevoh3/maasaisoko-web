...
class DashboardController extends Controller
{
    ...
    public function getDashboardData()
    {
        ...
        $top_selling_products = DB::select($selling_sql);
        ...
        $top_rating_products = DB::select($review_sql);
        $user = auth()->user();
        $kycStatus  = $user->kyc_status;                 // 'not_submitted' | 'pending' | 'verified' | 'rejected'
        $regFeePaid = (bool)$user->registration_fee_paid;
        $regFeeAmount = $user->classification === 'company'
            ? config('fees.registration.company')
            : config('fees.registration.individual');

        $regFeeCurrency = config('fees.registration.currency');


        return view('seller.dashboard', compact('total_order', 'pending_order', 'processing_order', 'ready_for_pickup_order', 'completed_order', 'canceled_order', 'published_product', 'review', 'top_selling_products',
            'top_rating_products','kycStatus','regFeePaid','user','regFeeAmount','regFeeCurrency'));
    }
}
