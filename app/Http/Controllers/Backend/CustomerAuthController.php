    ...
    public function CustomerLogin(Request $request)
    {
        ...
        if (!Auth::attempt($credentials, $request->boolean('remember'))) {
            return back()->withInput($request->only('email'))
                ->withFail(__('Your email address and password incorrect.'));
        }
        ...
        // Choose the default landing page by role
        $default = '/';
        if ($user->role_id == 1) {
        ...
        // If there was an intended URL, go there; otherwise use the role default
        return redirect()->intended($default)->withSuccess(__('Signed in'));
    }
